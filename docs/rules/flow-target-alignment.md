...
main path slopes up or down instead of reading as a straight horizontal line. The exception is a flow
touching a gateway or a boundary event, since those legitimately branch up and down.

## Why

A BPMN diagram is read far more often than its XML, by modelers, reviewers and business stakeholders
alike, and they follow it fastest when the main path is one straight horizontal line: each step drawn
on the same row as the one before it, the eye running from start to end without a jump. When a flow's
...
Adding `bpmn:SubProcess` exempts sub-processes entirely (inheritance-aware, so `bpmn:Transaction`
and `bpmn:AdHocSubProcess` too). You rarely need it: an expanded sub-process is already measured
against its inner start-event row, not the box centre (see [Scope](#scope)), so a horizontal flow
into a tall box is not reported.

## Examples

An order-approval process: start event, a review task, a split/merge gateway pair with two branch
tasks, an **expanded "Archive order" sub-process** and an end event. Both sub-processes below are the
same tall box; the only difference is where the inner reading line sits:
